...
m1 = model.matrix(outcome_28d ~ Age + Viremia -1, data=dfData)
...
## second formulation without interaction
...
rm(stanDso)
dfData$av = dfData$Age * dfData$Viremia
colnames(dfData)

oCV.1 = CCrossValidation.StanBern(dfData[,-3], dfData[,-3], dfData$outcome_28d,
...
## try second model
oCV.2 = CCrossValidation.StanBern(dfData[,-c(3,4)], dfData[,-c(3,4)], dfData$outcome_28d,
                                  dfData$outcome_28d, level.predict = '1',
                                  boot.num = 10, k.fold = 10, ncores = 2, nchains = 2) 
